...
def register_ROIs(A1, A2, dims, template1=None, template2=None, align_flag=True, 
    ...
    A2 = np.stack([a*(a>max_thr*a.max())/a.sum() for a in A2.T]).T 
    
    ## normalizing
    #A1 = scipy.sparse.lil_matrix(A1/A1.sum(axis=0))
    #A2 = scipy.sparse.lil_matrix(A2/A2.sum(axis=0))
    #print("Normalizing done , t = %5.3fs"%(time.time()-t_start))
    ...
    if D is None:
        ...
        #print("Obtained centers, t = %5.3f"%(time.time()-t_start))
        #A1_tr = A1.copy()
        #A2_tr = A2.copy()
        
        [D, scores, shifts] = distance_masks([A1, A2], [cm_1, cm_2], max_dist, dims=dims, std=std, cr=cr, enclosed_thr=enclosed_thr)
        
        #scores = [c,c_unshifted,c_shifted]
        #A1_tr = (A1 > 0).astype(float)
        #A2_tr = (A2 > 0).astype(float)
        #D = distance_masks_old([A1_tr, A2_tr], [cm_1, cm_2], max_dist, enclosed_thr=enclosed_thr)
        #shifts_matched = []
    
    #print("Calculating D done , t = %5.3fs"%(time.time()-t_start))
...
def register_multisession(A, dims, templates = [None], align_flag=True, 
                          ...
                          std = (5,5), cr = (15,15), w = 1/3,
                          max_dist=10, enclosed_thr=None, plot_results=False):
    """
    ...
    for sess in range(1,n_sessions):
        print("Now matching session %d to union up to session %d" % (sess+1,sess))
        #reg_results = register_ROIs(A[sess], A_union, dims, 
                                    ...
                                    #thresh_cost = thresh_cost, std=std, cr=cr, max_dist = max_dist, 
                                    #enclosed_thr = enclosed_thr, plot_results=plot_results)
        
        N_old = A_union.shape[-1]
        
        if 'csc_matrix' not in str(type(A[sess])):
          print("Session %d contains no data. Skipping to next..."%sess)
          
          N_add = 0
          sc = []
          
          new_match = np.zeros(A_union.shape[-1], dtype = int)*np.nan   # now matches here
          new_shifts = np.zeros([A_union.shape[-1],2])*np.NaN
          
        else:
          reg_results = register_ROIs(A_union, A[sess], dims, 
                                    template1 = templates[0], 
                                    template2 = templates[sess], align_flag=True, 
                                    max_thr = max_thr, use_opt_flow = use_opt_flow,
                                    thresh_cost = thresh_cost, std=std, cr=cr, max_dist = max_dist, 
                                    enclosed_thr = enclosed_thr, plot_results=plot_results)
        
        
          mat_un, mat_sess, nm_un, nm_sess, performance, A2, sc, shifts_mat = reg_results
          
          print('matching done, time passed: %5.3fs'%(time.time()-t_start))
          print(performance)
          ...
          A_handover = w*A_union[:,mat_un] + (1-w)*A2[:,mat_sess]
        
          A_union = A_union.asformat('lil')
          A_union[:,mat_un] = A_handover
          A_union = scipy.sparse.hstack([A_union, A2[:,nm_sess]]).asformat('csc')
          print('A union contains now %d ROIs' % A_union.shape[-1])
          
          new_match = np.zeros(A_union.shape[-1], dtype = int)*np.nan
          new_match[mat_un] = mat_sess
          new_match[N_old:N_old+N_add] = nm_sess      #range(N_old,A_union.shape[-1])
          
          new_shifts = np.zeros([A_union.shape[-1],2])*np.NaN
          new_shifts[mat_un] = shifts_mat
        
        scores.append(sc)
        matchings.append(new_match.tolist())
        shifts.append(new_shifts.tolist())
        
    assignments = np.zeros((A_union.shape[-1], n_sessions))*np.nan
    for sess in range(n_sessions):
        #assignments[matchings[sess],sess] = range(len(matchings[sess]))
        assignments[:len(matchings[sess]),sess] = matchings[sess]
        if sess < n_sessions-1:
          shifts[sess].extend([[np.NaN,np.NaN]]*(A_union.shape[-1]-len(shifts[sess])))
        
    shifts = np.array(shifts).transpose(1,0,2)
...
def distance_masks(M_s, cm_s, max_dist, std=None, cr=None, dims=None, enclosed_thr=None):
    """
    ...
    Args:
        M_s: tuples of 1-D arrays
            The A matrices to compare, containing spatial footprints
    ...
    ###
    ### dims: shape of FoV
    ### std: standard deviation of gaussian in x,y direction
    ### cr:   crop radius 
    
    ### furthermore, add to output:
    ###
    ### shift: proposed shift of ROI to match best with partner
    ...
    gt_n = M_s[0].shape[1]
    test_n = M_s[1].shape[1]
    
    if dims is None:
      dims = (sqrt(M_s.shape[0]),sqrt(M_s.shape[0]))
    
    shifts = np.zeros([gt_n,test_n,2])
    
    # precompute norms to save time
    chunks = 500
    
    gt_norm = np.zeros(gt_n)
    gt_steps = int(gt_n/1000)
    for i in range(gt_steps):
      n_start = i*chunks
      n_end = min(gt_n,(i+1)*chunks)
      gt_norm[n_start:n_end] = np.linalg.norm(np.array(M_s[0][:,n_start:n_end].todense()),axis=0)
    
    test_norm = np.zeros(test_n)
    test_steps = int(test_n/1000)
    for i in range(test_steps):
      n_start = i*chunks
      n_end = min(test_n,(i+1)*chunks)
      test_norm[n_start:n_end] = np.linalg.norm(np.array(M_s[1][:,n_start:n_end].todense()),axis=0)
      
      
    gt_norm = np.linalg.norm(np.array(M_s[0].todense()),axis=0)
    test_norm = np.linalg.norm(np.array(M_s[1].todense()),axis=0)
    
    c = scipy.sparse.lil_matrix((gt_n,test_n))
    c_unshifted = scipy.sparse.lil_matrix((gt_n,test_n))
    c_discounted = scipy.sparse.lil_matrix((gt_n,test_n))
    #refused = 0
    C_gauss = makeGaussian(cr,std)      #generate 2-d gaussian with given variance, etc
        ...
        for i in range(nb_gt):
            ...
            for j in range(nb_test):  # for each components on the tests
                ...
                if dist < max_dist:
                    ...
                    #test_norm = np.linalg.norm(test_comp[:,j].todense())
                    #D[i, j] = 1 - np.array(gt_comp[:, i].T.dot(test_comp[:, j]).todense()).squeeze()/(gt_norm*test_norm)
                    
                    cm = (cmgt_comp[i] + cmtest_comp[j])/2
                    [c[i,j], c_unshifted[i,j], c_discounted[i,j], shifts[i,j,:]] = shift_match(gt_comp[:,i], test_comp[:,j], gt_norm[i], test_norm[j], dims, C_gauss, cm=cm, cr=cr)
                    
                    D[i, j] = 1-c_discounted[i,j]
                    ...
                    if (np.linalg.norm(shifts[i,j,:]) < 2) and (c_discounted[i,j] > 0.9):
                        break
                else:
                    D[i, j] = 1
        
        ### avoid competing for best match, when proper match is already found
        #for i in range(nb_gt):
          #for j in range(nb_test):  # for each components on the tests
            #if D[i,j] < 0.1:
              
              #D[i,np.where(D[i,:] > 0.3)] = 1
              #D[np.where(D[:,j] > 0.3),j] = 1
        
        D_s.append(D)
    #print('refused: %d'%refused)
    return D_s, [c, c_unshifted, c_discounted], shifts
  
  
  
def distance_masks_old(M_s, cm_s, max_dist, enclosed_thr=None):
    """
    Compute distance matrix based on an intersection over union metric. Matrix are compared in order,
    with matrix i compared with matrix i+1
...
def shift_match(A1, A2, A1_norm, A2_norm, dims, C_gauss, cm=None, cr=np.array([15,15])):
  
  ### invalues:
  ### A1,A2:    footprints, for which the shift and 
  ### cm :     center of mass, around which to crop
  ### cr :      crop radius
  ### shift:    introduce shift between the footprints (for testing purposes)
  
  ### outvalues:
  ### C_max :   discounted spatial footprint
  ### x_shift : proposed shift to apply in x-direction
  ### y_shift : proposed shift to apply in x-direction
  
  if C_gauss is None:
    C_max = A1.multiply(A2).sum()/(A1_norm*A2_norm)
    return C_max, C_max, C_max, np.zeros(2)
  
  A1 = A1.reshape(dims[0],dims[1]).todense()
  A2 = A2.reshape(dims[0],dims[1]).todense()
  
  ### maybe remove this whole part
  cm = np.flipud(cm)
  
  extent = np.floor(np.array([cm-cr,cm+cr+1])).astype(int)
  if (extent<0).any() or (extent>dims).any():   ## window has to be cropped
    
    # crop gaussian
    ext_min = -np.minimum(extent[0],0)
    ext_max = np.maximum(extent[1],dims)-dims
    C_gauss = C_gauss[ext_min[0]:(2*cr[0]+1)-ext_max[0],ext_min[1]:(2*cr[1]+1)-ext_max[1]]
    #print('crop:'+str(ext_min)+', '+str(ext_max))
    extent = np.maximum(extent,0)
    extent = np.minimum(extent,dims)
  
  A1_crop = A1[extent[0][0]:extent[1][0],extent[0][1]:extent[1][1]]
  A2_crop = A2[extent[0][0]:extent[1][0],extent[0][1]:extent[1][1]]
  
  C = np.fft.fftshift(np.real(np.fft.ifft2(np.fft.fft2(A1_crop) * np.fft.fft2(np.rot90(A2_crop,2)))))/(A1_norm*A2_norm)
  C_max = np.max(C)
  C_raw = C[cr]
    
  C_total = C_gauss*C
  C_max_discounted = np.max(C_total)
  
  if np.isnan(C_max):
    C_max = 0
  
  if C_max < 0.5:   ## no proper shift could be found
    shifts = [0,0]
  else:
    shifts = np.subtract(np.where(C==C_max),cr).T[0].astype(int)
  
  #if (np.abs([x_shift, y_shift])>3).any():
    #pl.close('all')
    #print('---------------- strong shift ---------------')
    #print(type(C))
    #print(type(C_gauss))
    
    #print(C[cr[0]-3:cr[0]+4,cr[1]-3:cr[1]+4])
    #print(C_gauss[cr[0]-3:cr[0]+4,cr[1]-3:cr[1]+4])
    #print(C_total[cr[0]-3:cr[0]+4,cr[1]-3:cr[1]+4])
    #print('C: '+ str(np.max(C_gauss)) + ', ' + str(np.max(C)) + ', ' + str(C_max))
    ...
    #(rows, cols) = A2_crop.shape[:2] 
    ##M = np.float32([[1, 0, y_shift_disc], [0, 1, x_shift_disc]])
    ##A2_shift = cv2.warpAffine(A2_crop, M, (cols, rows)) 
    
    ##pl.subplot(2,2,3)
    ##pl.imshow(A1_crop + A2_shift)
    ...
    #input('waiting...')
  return C_max, C_raw, C_max_discounted, shifts
  
  

def makeGaussian(cr,std):
    """ Make a square gaussian kernel.
    cr is the half length of the sides of the rectangle
    std are the standard deviations in x, y direction.
    """
    if (std is None) or (cr is None):
      return None
    else:
      x = np.arange(-cr[0],cr[0]+1,1)
      y = np.arange(-cr[1],cr[1]+1,1)
      y = y[:,np.newaxis]
      
      return np.exp(- (x**2/(2*std[0]**2) + y**2/(2*std[1]**2)))