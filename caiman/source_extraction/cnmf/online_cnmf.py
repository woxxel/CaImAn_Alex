...
#        print(np.max(1/scipy.sparse.linalg.norm(self.estimates.Ab,axis = 0)))
    ...
    def initialize_online(self):
        ...
        Y = caiman.load(fls[0], subindices=slice(0, opts['init_batch'],None))#.astype(np.float32)
        ...
        #[T, d1, d2] = Y.shape
        #Yr = np.reshape(Y, (T, d1 * d2), order='F').T
...
#                cnm.estimates.f, cnm.estimates.S, cnm.estimates.YrA
    ...
    def fit_online(self, **kwargs):
        ...
        if self.params.get('online', 'save_online_movie'):
            ...
            print(self.params.get('online', 'movie_name_online'))
            out = cv2.VideoWriter(self.params.get('online', 'movie_name_online'),
                                  fourcc, self.params.get('data','fr'), tuple([int(2*x) for x in self.params.get('data', 'dims')]))
        for iter in range(epochs):
            ...
            for file_count, ffll in enumerate(process_files):
                Y_ = caiman.load(ffll, subindices=slice(init_batc_iter[file_count], None, None))#.astype(np.float32)
                ...
                for frame_count, frame in enumerate(Y_):   # process each file
                    ...
                    if t % 1000 == 0:
                        print('Epoch: ' + str(iter + 1) + '. ' + str(t) +
                              ...
                              + str(self.estimates.Ab.shape[-1] - self.params.get('init', 'nb')) + '. \t Time spent:' + str(time()-t_start))
                        old_comps = self.N
                    ...
                    if self.params.get('online', 'motion_correct'):    # motion correct
                        ...
                        if self.params.get('motion', 'pw_rigid'):
                            #frame_cor1, shift = motion_correct_iteration_fast(frame_, templ, max_shifts_online, max_shifts_online)
                    ...
                    if self.params.get('online', 'show_movie'):
                        self.t = t
                        vid_frame = self.create_frame(frame_cor,show_residuals=False)
                        if self.params.get('online', 'save_online_movie'):
                            out.write(vid_frame)
    ...
    def create_frame(self, frame_cor, show_residuals=True, resize_fact=1):
        ...
        if show_residuals:
            ...
            all_comps = np.array(A.sum(-1)).reshape(self.dims, order='F')/np.max(A.sum(-1))
                                                  # spatial shapes
        ...
        cv2.putText(vid_frame, captions[3], (np.int(self.dims[0] * resize_fact) + 5, np.int(
            self.dims[1] * resize_fact) + 20), fontFace=5, fontScale=0.8, color=(0, 255, 0), thickness=1)
        cv2.putText(vid_frame, 'Frame = ' + str(self.t) + ', # ROIs = ' + str(self.N), (vid_frame.shape[1] // 2 - vid_frame.shape[1] //
