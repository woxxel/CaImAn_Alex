...
def load(file_name,fr=15,start_time=0,meta_data=None,subindices=None,shape=None,
         var_name_hdf5 = 'mov', in_memory = False, is_behavior = False, bottom=0,
         top=0, left=0, right=0, channel = None, outtype=np.uint16, as_movie=True):
    """
...
#                    return movie(f[var_name_hdf5][subindices], **attrs).astype(outtype)
            ...
            Yr, dims, T = load_memmap(os.path.join(
                os.path.split(file_name)[0], filename), data_type=outtype)
