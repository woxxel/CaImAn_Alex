...
def cnmf_patches(args_in):
    ...
    import logging
    from . import cnmf
    file_name, idx_, shapes, params, data_type = args_in
    ...
    Yr, dims, timesteps = load_memmap(file_name,data_type=data_type)
...
def run_CNMF_patches(file_name, shape, params, gnb=1, dview=None, memory_fact=1,
                     ...
                     indeces=[slice(None)]*3, data_type=np.float32):
    ...
    for id_f, id_2d in zip(idx_flat, idx_2d):
        #        print(id_2d)
        args_in.append((file_name, id_f, id_2d, params_copy, data_type))
        if del_duplicates:
