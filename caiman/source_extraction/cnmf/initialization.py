...
def initialize_components(Y, K=30, gSig=[5, 5], gSiz=None, ssub=1, tsub=1, nIter=5, maxIter=5, nb=1,
    ...
    if normalize_init is True:
        logging.debug('Noise Normalization')
        if img is None:
    ...
    if ssub != 1 or tsub != 1:
        ...
        if method == 'corr_pnr':
            logging.debug("Spatial Downsampling 1-photon")
            ...
            logging.debug("Spatial Downsampling 2-photon")
            # this icrements the performance against ground truth and solves border problems
...
#            Y_ds = downscale_local_mean(Y, tuple([ssub] * len(d) + [tsub]), cval=mean_val) # this gives better results against ground truth for 2-photon datasets
    ...
    if method == 'greedy_roi':
        ...
        if use_hals:
            logging.debug('(Hals) Refining Components...')
...
def ICA_PCA(Y_ds, nr, sigma_smooth=(.5, .5, .5), truncate=2, fun='logcosh',
    ...
    logging.debug("not a function to use in the moment ICA PCA \n")
...
def sparseNMF(Y_ds, nr, max_iter_snmf=500, alpha=10e2, sigma_smooth=(.5, .5, .5),
    ...
    if remove_baseline:
        ...
        logging.debug('NOT REMOVING BASELINE')
        bl = np.zeros(m.shape[1:])
        m1 = m
...
def greedyROI(Y, nr=30, gSig=[5, 5], gSiz=[11, 11], nIter=5, kernel=None, nb=1,
    ...
    """
    logging.debug("Greedy initialization of spatial and temporal components using spatial Gaussian filtering")
    d = np.shape(Y)
    Y[np.isnan(Y)] = 0
    ...
    if rolling_sum:
        logging.debug('USING ROLLING SUM FOR INITIALIZATION....')
        ...
        logging.debug('USING TOTAL SUM FOR INITIALIZATION....')
    ...
    for k in range(nr):
        ...
        if k < nr - 1:
            ...
            if rolling_sum:
                ...
                v[tuple([slice(*a) for a in Mod])] = np.sum(rho[tuple([slice(*a)
...
def greedyROI_corr(Y, Y_ds, max_number=None, gSiz=None, gSig=None, center_psf=True,
    ...
    logging.debug('One photon initialization..')
    ...
    if ring_size_factor is not None:
        ...
        logging.debug('Compute Background')
        ...
        logging.debug('Merge Components')
        ...
        logging.debug('Compute Background Again')
        # background according to ringmodel
        W, b0 = compute_W(Y_ds.reshape((-1, total_frames), order='F'),
    ...
    use_NMF = True
    if nb == -1:
        logging.debug('Return full Background')
        b_in = spr.eye(len(B), dtype='float32')
        f_in = B
    elif nb > 0:
        logging.debug('Estimate low rank Background')
        if use_NMF:
            model = NMF(n_components=nb, init='nndsvdar')
